...
func GeneratePatternDescription(UserInfo User) string {
	return fmt.Sprintf("Your current file naming pattern is `%s`. For example, this might generate the filename `%s`.", UserInfo.NamingScheme, CreateFilename(UserInfo.NamingScheme))
}
